...
namespace pili_sdk_csharp.pili_qiniu
{
    public class Credentials
    {
        ...
        public static string signUrl(string ak,string sk,string domain,string hubname,string streamkey,int expire)
        {
            string path = string.Format("/{0}/{1}?e={2}",hubname,streamkey,expire);
            string signdata = Credentials.sign(sk, path);
            string token = ak + ":" + signdata;
            string rtmpPublishUrl = string.Format("rtmp://{0}{1}&token={2}", domain, path, token);
